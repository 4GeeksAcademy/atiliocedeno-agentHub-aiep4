...
export default function AgentCard({ agent, onDelete }: AgentCardProps) {
  ...
  return (
    <>
      <div className="bg-white dark:bg-[#1E293B] rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-[#334155]/30 flex flex-col gap-4 hover:border-primary/30 transition-all duration-300 group">
        {/* Header */}
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-gray-900 dark:text-white font-semibold">{agent.name}</h3>
      ...
      <Modal open={detailOpen} onClose={() => setDetailOpen(false)} title={agent.name}>
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3">
            <div className="w-16 h-16 bg-primary/10 dark:bg-primary/20 rounded-2xl flex items-center justify-center text-3xl">
              {agent.icon}
            </div>
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{agent.description}</p>
            </div>
          </div>
          <div>
      ...
      <Modal open={confirmOpen} onClose={() => setConfirmOpen(false)} title="Eliminar agente">
        <p className="text-gray-500 dark:text-gray-400 mb-6">
          ¿Estás seguro de que deseas eliminar a <strong className="text-gray-900 dark:text-white">{agent.name}</strong>? Esta acción no se puede deshacer.
        </p>
        <div className="flex gap-3 justify-end">
          <button
            onClick={() => setConfirmOpen(false)}
            className="px-4 py-2 rounded-lg bg-gray-200 dark:bg-[#334155] text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-[#475569] transition-colors"
          >
            Cancelar
          </button>
          <button
            ...
            className="px-4 py-2 rounded-lg bg-red-500/10 text-red-500 dark:bg-red-500/20 dark:text-red-400 hover:bg-red-500/20 dark:hover:bg-red-500/30 transition-colors"
          >
            Eliminar
          </button>
        </div>
